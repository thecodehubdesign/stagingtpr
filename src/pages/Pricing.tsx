...
const Pricing = () => {
  const membershipPlans = [
    ...
    },
    {
      ...
      features: [
        "4 classes to use anytime",
        "Valid for 6 weeks",
        "Mix and match any classes",
        "All studio locations",
        "Free grip aids included"
      ],
      popular: true,
      icon: Heart,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {membershipPlans.map((plan, index) => (
              <Card 
                key={index}
                className={`cyber-card p-8 relative animate-fade-in ${plan.popular ? 'ring-2 ring-fuchsia-500 scale-105' : ''}`}
                style={{ animationDelay: `${index * 0.15}s` }}
              >
                {plan.popular && (
                  <Badge className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-fuchsia-500 text-white px-4 py-1">
                    Most Popular
                  </Badge>
                )}
                ...
                <Button 
                  className={`w-full ${plan.popular ? 'neon-button text-black font-bold' : 'bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-500 hover:to-gray-600'}`}
                >
